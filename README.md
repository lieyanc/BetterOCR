...
2. **判据必须可观测。** 不采信模型自报的置信度:那个数聚集在 0.9/0.95/0.98
   ...
   引擎逐字给出同一文本、多少个持异议、这行走了哪条路径。
3. **强模型成本与分歧量成正比。** 共识行零成本通过;一张图的全部分歧
   打包成**一次**仲裁调用,并附上下文行帮助强模型在图中定位。
4. **全程确定性。** 引擎按名排序、对齐平局有固定优先级、代表候选取 medoid
...
```json
{
  ...
  "candidates": ["…各引擎原始行级结果(纯文本行),含失败者的 err…"]
}
```
...
引擎只返回纯文本行,不返回任何自报指标。`confidence` 全部由结构信号推导,
因此可复现,也不额外消耗 token:
...
- **`consensus`** — 由逐字一致的引擎数 `k` 与有效引擎总数 `n` 决定。一致引擎
  ...
  共识判据本身是 `k ≥ 2 且 2k > n`。分母取**全体有效引擎数**而非该行的候选
  数:某个引擎在这一行完全没有产出候选,本身就是"这行可能不存在"的证据。
- **`escalated`** — 裁定与某个引擎候选逐字吻合(拿到独立旁证)记 0.95,
  与所有候选都不同(只有仲裁器一家之言)记 0.80。
- **`fallback`** — 由候选彼此的接近程度线性映射到 `[0.30, 0.65]`;只有一个
  引擎看到的孤行记 0.25。上限刻意压在 0.7 以下:兜底是"没得选才选它",
  不该显示得和共识行一样可信。
